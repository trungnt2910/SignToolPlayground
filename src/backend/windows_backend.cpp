...
{
    throw std::runtime_error(message + " (HRESULT " + std::to_string(static_cast<unsigned long>(hr)) + ")");
}
...
UniqueCertContext FindCertificateBySubject(const HCERTSTORE store, const std::wstring& subject_name)
{
    PCCERT_CONTEXT certificate = CertFindCertificateInStore(
        ...
        nullptr);
    if (certificate == nullptr) {
        throw std::runtime_error("Could not find a matching signing certificate by subject name.");
    }
...
void EnsurePrivateKeyIsAvailable(const CERT_CONTEXT* certificate)
{
    ...
    if (!CryptAcquireCertificatePrivateKey(certificate, 0, nullptr, &key_handle, &key_spec, &must_free)) {
