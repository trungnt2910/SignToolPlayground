...
std::wstring Win32Helper::utf8ToWide(const std::string& utf8Str)
{
    if (utf8Str.empty())
    {
        return L"";
    ...
    if (sizeNeeded <= 0)
    {
        throw Win32Exception("Failed to convert UTF-8 string to wide string.");
    }
...
std::string Win32Helper::wideToUtf8(const std::wstring& wideStr)
{
    if (wideStr.empty())
    {
        return "";
    ...
    if (sizeNeeded <= 0)
    {
        throw Win32Exception("Failed to convert wide string to UTF-8 string.");
    }
